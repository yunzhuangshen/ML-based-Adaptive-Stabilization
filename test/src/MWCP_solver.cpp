...
namespace GCP {
    ...
    void MWCP_solver::solve_mwc_tsm(){
        ...
        char c[0];
        best_obj = solver.tsm(c, name2, nb_node, nb_edge, cutoff, AdjacentList, Node_Degree, Node_Weight, Node_Bound);
        objs = solver.sol_objs; sols = solver.sols;
        isOptimal = solver.isOptimal;
        exact_rc = 1 - best_obj/1e12;
        // cout << isOptimal << " " << best_obj << " " << exact_rc << "\n";
    }
    ...
    void MWCP_solver::postprocessing(){
        ...
        for (auto i = 0; i < sols_distinct.size(); i++){
            if (isValid[i]){
                ...
                new_col_rc_values.push_back(rc);
                new_cols.push_back(sols_distinct[i]);
            }
        }

        num_neg_rc_col  = new_col_rc_values.size();
        if (method==8 && num_neg_rc_col > 0 && isOptimal){
            int best_col=-1; double best_rc = 1e2;
            for (auto i = 0; i < num_neg_rc_col; i++){
                if (new_col_rc_values[i] < best_rc){
                    best_rc = new_col_rc_values[i];
                    best_col = i;
                }
            }
            optimal_mis = new_cols[best_col];
            assert(optimal_mis.size() > 0);
        }
    }
